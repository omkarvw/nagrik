...
function MedicinesPage({ medicines, loading, onSelectMedicine }: {
  medicines: Medicine[];
  loading: boolean;
  onSelectMedicine: (medicine: Medicine) => void;
}) {
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-12">
      <section className="mb-8 md:mb-16">
        ...
        {loading ? (
          ...
          <MedicinesList medicines={medicines} onSelect={onSelectMedicine} />
        )}
      </section>
    </div>
  );
...
function AppContent() {
  ...
  const handleSelectMedicine = (medicine: Medicine) => {
    logSelectMedicine(medicine.id, medicine.genericName);
    navigate('/');
    // Scroll to comparison section after navigation
    setTimeout(() => {
      const element = document.getElementById('comparison-section');
      if (element) {
        element.scrollIntoView({ behavior: 'smooth', block: 'start' });
